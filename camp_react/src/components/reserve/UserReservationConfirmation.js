...
function UserReservationConfirmation(props) {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/reserve/userReservationList/${props.userInfo.memberIdx}`)
      .then(res => {
  ...
  const cancelReservation = (impUid) => {
    ...
    // 예약 테이블 상태 결제 취소로 수정 //테스트용 pg사 자정에 취소시
  ...
  useEffect(() => {
    if (cancelPaymentInfo) {
      setReservationList(prevList => prevList.map(item => {
        if (item.idx === paymentInfo.reservationIdx) {
          return {
            ...item,
            payStatus: "결제취소"
          };
  ...
  return (
    <div className={"my-5"}>
      <div className={"my-5"}>
        <h4 className={"text-center"}>내 예약 목록</h4>
        ...
        <div>
          <table className={"table table-hover"}>
            ...
            {reservationList.length === 0 ?
              <tr>
                <td colSpan={14}>예약 내역이 없습니다.</td>
              </tr>
              : reservationList.map(item => {
                return (
                  <tr key={item.idx}>
                    {/*캠핑장 링크 추후 수정*/}
                    {/*<td><Link to={`/camp#${item.userCampMainIdx}`}*/}
                    {/*          className={"btn-confirm btn-confirm-primary"}>바로가기</Link></td>*/}
                    <td>{item.name}</td>
                    ...
                    {item.payStatus === "결제완료" || item.payStatus === "결제취소" ? <td>
                      <button className={"btn-confirm btn-confirm-primary"} data-bs-toggle="modal"
                              data-bs-target="#paymentModal" onClick={() => {
                        getPaymentInfo(item.idx)
                      }}>결제내역
                      </button>
                    </td> : <td>-</td>}
                    {item.payStatus === "결제완료" ? <td>
                      <button className={"btn-confirm btn-confirm-primary"} onClick={() => {
                ...
                <div className="modal-body">
                  {paymentInfo &&
                    <table className={"table table-hover"}>
                      ...
                      <tr>
                        <td>온라인 영수증</td>
                        <td><a href={paymentInfo.receiptUrl} className={"text-decoration-none"} target={"_blank"}>온라인
                          영수증 바로가기</a></td>
                      </tr>
                      </tbody>
                    </table>
