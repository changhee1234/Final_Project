...
// @Entity : JPA에서 해당 클래스가 데이터베이스 테이블이라는 것을 알려주는 어노테이션
// 클래스명이 테이블명으로 사용됨
// DB의 테이블명을 변경하고자 할 경우 @Table 어노테이션을 사용해야 함
// Mybatis 사용 시, DTO 클래스의 역할도 함께 하고 있음
...
// 옵션이 4가지가 존재함
// 1)AUTO : 선택한 데이터베이스에 따라 자동으로 선택
// 2)IDENTITY : 기본 키 생성을 데이터베이스에 위임(데이터베이스 설정에 따름)
// 3)SEQUENCE : 데이터 베이스 시퀀스를 사용하여, 기본 키를 설정(oracle에서 사용함)
// 4)TABLE : 키 생성 테이블을 사용함
  @Id
...
// 컬럼이 가지고 있는, 특성을 적용하고자할 경우 사용
// 기본적으로 자바 클래스에서 카멜명명법 사용 시, 데이터베이스에서 스네이크 명명법으로 적용
// unique: 컬럼의 유니크 여부 설정
// nullable : 컬럼의 null 허용 여부
// updatable : 컬럼의 수정 가능 여부 설정
// length : 컬럼에 저장된 데이터의 크기 설정
